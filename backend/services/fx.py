...
def get_hedge_coverage_data(currency: str = "INR", fy: str = "All") -> Dict[str, Any]:
    ...
    rows = fetch_dict(f"""
        SELECT COALESCE(TRIM({COL_MAP['product_name']}), 'Unknown') AS product, COALESCE({COL_MAP['type']}, 'Unknown') AS type,
            CASE WHEN COALESCE({COL_MAP['type']},'') = 'CAPEX' THEN 'Hedged' ELSE 'Unhedged' END AS hedge_status,
            COUNT(*) AS lc_count, SUM({amt_inr}) AS unpaid_inr, SUM({amt_fc}) AS unpaid_fc, MAX(COALESCE({COL_MAP['currency']}, 'USD')) AS currency
