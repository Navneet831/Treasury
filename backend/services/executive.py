...
@ttl_cache(seconds=60)
def get_command_data(currency: str = "INR", fy: str = "All", payment_status: str = "Unpaid", facility_type: str = "LC") -> Dict[str, Any]:
    cd, amt_col = get_current_date(), COL_MAP["amt_inr"] if currency == "INR" else COL_MAP["amt_fc"]
    fy_filter = get_fy_clause(fy, COL_MAP['op_date'])
    
    # Facility filter for LC table
    fac_filter = " AND \"SBLC Status\" LIKE 'Yes%'" if facility_type == "SBLC" else " AND (\"SBLC Status\" NOT LIKE 'Yes%' OR \"SBLC Status\" IS NULL)"
